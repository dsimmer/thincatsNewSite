...
function pfAjaxRequest(element) {  
    ...
    if (arg.search("method_name=SaveEmailTemplate")) {
        pfAjaxRequest2(bindElement);
        return;
    }
...
function pfAjaxRequest2(element) {
    ...
    for( i = 0; i < _arg.length/2; i++ ) {
        data2 = data2 + _arg[i] + "&";
        num++;
    }

    data2 = data2 + "method_name=SaveEmailTemplate";

    pfAjaxCall( bindElement, 'pf_ajax_request', data2, function(data) {
        bindElement.after("<div class='pf_ajax_result'>"+data+"</div>");        
    });

    data2 = "";
    for( i = num; i < _arg.length; i++ ) {
        data2 = data2 + _arg[i] + "&";
        num++;
    }
...
function umPageNavi(pageID, isNext, element) {
    ...
    jQuery("html, body").animate({
        scrollTop: 0//jQuery("#um_page_segment_" + pageID).offset().top
    });
}
